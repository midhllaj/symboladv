...
export const fragmentShader = `
  ...
  void main() {
    ...
    // Text Logic (Title/Year)
    float textHeight = 0.08;
    float textY = 0.88; // Position at bottom
    
    bool inTextArea = cellUV.x >= 0.05 && cellUV.x <= 0.95 && cellUV.y >= textY && cellUV.y <= (textY + textHeight);
    
    // Texture Index Calculation (Repeating pattern)
    // We used a 3.0 offset factor in the JS, using consistent logic here
    ...
    if (inImageArea && imageAlpha > 0.0) {
      ...
      atlasUV.y = 1.0 - atlasUV.y; // Flip Y for texture
    ...
    if (inTextArea) {
      ...
      // Mix text color on top of background
